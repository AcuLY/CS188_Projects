...
def findModelUnderstandingCheck() -> Dict[Expr, bool]:
    ...
    """
    a = Expr('A')
    a.__dict__ = {'op': 'a', 'args': ()}
    return {a: True}

def entails(premise: Expr, conclusion: Expr) -> bool:
    """Returns True if the premise entails the conclusion and False otherwise.
    """
    return not findModel(premise & ~conclusion) # 如果 p 真 q 假则不可能 p 蕴含 q
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    """
    return pl_true(~inverse_statement, assignments) # ~p 蕴含 q
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    inverseLiterals = [~expr for expr in literals]
    comb = itertools.combinations(inverseLiterals, 2)
    return conjoin([disjoin(list(exprs)) for exprs in comb])    # (~A | ~B) & (~A | ~C) & (~B | ~C)
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    pacphysics_sentences.append(conjoin([PropSymbolExpr(wall_str, x, y) >> ~PropSymbolExpr(pacman_str, x, y, time=t) for (x, y) in all_coords]))    # 是墙蕴含不在该格子
    pacphysics_sentences.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=t) for x, y in non_outer_wall_coords]))   # 在一个格子上
    pacphysics_sentences.append(exactlyOne([PropSymbolExpr(action, time=t) for action in DIRECTIONS]))  # 采取一个行动
    if sensorModel:
...
def positionLogicPlan(problem) -> List:
    ...
    # 初始状态
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=0) for (x, y) in non_wall_coords])) # t = 0 时不在其他地方
    KB.append(exactlyOne([PropSymbolExpr(action, time=0) for action in actions]))
    # 所有后继
    for t in range(1, 50):
        # 可能在的位置，并保证该位置能合法到达
...
def foodLogicPlan(problem) -> List:
    ...
    # 初始状态
    KB.extend([PropSymbolExpr(food_str, x, y, time=0) for (x, y) in food])  # 食物状态
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=0) for (x, y) in non_wall_coords])) # t = 0 时不在其他地方
    KB.append(exactlyOne([PropSymbolExpr(action, time=0) for action in actions]))
    # 所有后继
    for t in range(1, 50):
        # 可能在的位置，并保证该位置能合法到达
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=t) for (x, y) in non_wall_coords]))
        KB.extend([pacmanSuccessorAxiomSingle(x, y, t, walls) for (x, y) in non_wall_coords])
        # t 时某处没食物当且仅当 t-1 时吃豆人在那里或者 t-1 时那里就没有食物
        KB.extend([~PropSymbolExpr(food_str, x, y, time=t) % (PropSymbolExpr(pacman_str, x, y, time=t-1) | ~PropSymbolExpr(food_str, x, y, time=t-1)) for (x, y) in food])
        # 边界条件为全部格子都没有食物
...
def localization(problem, agent) -> Generator:
    ...
    KB.append(conjoin([~PropSymbolExpr(wall_str, x, y) for (x, y) in all_coords if (x, y) not in walls_list]))
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        # 跟上一问大致相同，不过初始只知道外围一圈是墙，需要推理内部的墙
